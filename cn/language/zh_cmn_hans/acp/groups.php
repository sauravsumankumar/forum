...
$lang = array_merge($lang, array(
	'ACP_GROUPS_MANAGE_EXPLAIN'		=> '在这个面板您可以管理所有的用户组。您可以删除、创建和编辑现有的用户组。此外，您还可以选择组领导，切换组状态为开放、隐藏或关闭，以及设置组名与描述。',
	'ADD_GROUP_CATEGORY'			=> '添加类别',
	'ADD_USERS'						=> '添加用户',
	'ADD_USERS_EXPLAIN'				=> '这里您可以添加新用户到用户组。您可以选择是否使用这个用户组作为用户的默认用户组。并且您可以设置用户组领导。如果有多个用户请在一行只输入一个用户名。',
	...
	'GROUP_DESC'					=> '用户组描述',
	'GROUP_DETAILS'					=> '用户组细节',
	'GROUP_EDIT_EXPLAIN'			=> '这里您可以编辑存在的用户组。更改组名称，描述和类型 (开放，封闭等)。您还可以设置一些用户组范围的选项例如颜色，等级。更改将覆盖用户的当前设置。请注意组成员可以更改他们的头像，除非您设置了相关的权限。',
	...
	'GROUP_MESSAGE_LIMIT_EXPLAIN'	=> '这个设置会覆盖用户的短信限制。所有用户组的最大值将决定实际值。<br />如果设成 0 将使用全局设定中的默认设置来覆盖该组用户的设置。',
	...
	'GROUP_MAX_RECIPIENTS_EXPLAIN'  => '单个私人短信允许发送的收件人数量上限。用户组的最大值将决定实际值。<br />如果设成 0 将使用全局设定中的默认设置来覆盖该组用户的设置。',
	'GROUP_OPTIONS_SAVE'			=> '组选项',
	'GROUP_PROMOTE'					=> '升级为组领导',
	'GROUP_RANK'					=> '组级别',
	'GROUP_RECEIVE_PM'				=> '用户组可以接收私人短信',
	'GROUP_RECEIVE_PM_EXPLAIN'		=> '请注意隐藏的用户组不可以接收短信，这里的设置不起作用。',
	'GROUP_REQUEST'					=> '申请',
	'GROUP_SETTINGS_SAVE'			=> '组设置',
	...
	'GROUP_USERS_INVALID'			=> '没有用户被加入到用户组，因为下面的用户不存在： %s',
