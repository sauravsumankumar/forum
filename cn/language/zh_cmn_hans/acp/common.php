...
$lang = array_merge($lang, array(
	...
	'ACP_BACKUP'				=> '备份',
	'ACP_BAN'					=> '封禁',
	'ACP_BAN_EMAILS'			=> '封禁 email 地址',
	'ACP_BAN_IPS'				=> '封禁 IP 地址',
	...
	'ACP_EMAIL_SETTINGS'		=> 'Email 设定',
	'ACP_EXTENSION_GROUPS'		=> '管理扩展名类别',
	...
	'ACP_MANAGE_ATTACHMENTS_EXPLAIN'	=> '这里您可以列出和删除贴子和站内短信中的附件。',
	...
	'ACP_MASS_EMAIL'			=> '群发 email',
	'ACP_MESSAGES'				=> '站内短信',
	'ACP_MESSAGE_SETTINGS'		=> '站内短信设定',
	...
	'CANNOT_CREATE_SYMLINK'	=> '无法创建符号连接',
	'CANNOT_DELETE_FILES'	=> '无法从系统删除文件',
	'CANNOT_DUMP_FILE'		=> '无法清理文件',
	'CANNOT_MIRROR_DIRECTORY'	=> '无法镜像目录',
	'CANNOT_RENAME_FILE'	=> '无法在系统重命名一个文件',
	'CANNOT_TOUCH_FILES'	=> '无法确认文件是否存在',
	
	'CONTAINER_EXCEPTION'   => 'phpBB 在安装扩展的时候遇到一个容器生成错误，所有的扩展都临时被停用了。请清除论坛缓存。一旦这个错误解决，所有的扩展会自动启用。如果持续出现这个错误，请访问 <a href="https://www.phpbb.com/support">phpBB.com</a> 获取帮助。',
	'EXCEPTION' 			=> '异常',
	...
	'NOTIFY'				=> '通知',
	'NO_ADMIN'				=> '您未被授权管理此论坛。',
	'NO_EMAILS_DEFINED'		=> '没有发现可用的 email 地址',
	'NO_FILES_TO_DELETE'	=> '您选的要删除的附件不存在。',
	...
	'RELEASE_ANNOUNCEMENT'		=> '发行公告',
	'REMIND'							=> '提醒',
	'REPARSE_LOCK_ERROR'				=> '另外一个进程正在运行解析。',
	'RESYNC'							=> '重新同步',
...
// PHP info
$lang = array_merge($lang, array(
	'ACP_PHP_INFO_EXPLAIN'	=> '这个页面列出了服务器上所安装的 PHP 的信息。这包括装载的模块，可用的变量和默认设定，在诊断问题时这些信息可能会有用。请注意一些主机服务商可能会因为安全因素禁止显示某些信息。您尽可能不要泄露这些信息，除非技术支持中的 <a href="https://www.phpbb.com/about/team/">官方开发成员</a> 需要这些信息。',

	'NO_PHPINFO_AVAILABLE'	=> '无法获取 PHP 信息，Phpinfo() 因为安全原因被禁用。',
));

// Logs
$lang = array_merge($lang, array(
	'ACP_ADMIN_LOGS_EXPLAIN'	=> '这里列出了论坛管理员执行的操作。您可以按用户名、日期、IP 地址或操作排序。如果您有合适的权限，您也可以清除部分或全部日志记录。',
	'ACP_CRITICAL_LOGS_EXPLAIN'	=> '这里列出了论坛自身的操作。这些日志为您解决特定问题提供有用信息，例如无法投递的 email。您可以按用户名、日期、IP 地址或操作排序。如果您有足够的权限，您也可以清除部分或全部日志记录。',
	'ACP_MOD_LOGS_EXPLAIN'		=> '这里列出了论坛版主执行的操作，在下拉框中选择一个版面。您可以按用户名、日期、IP 地址或操作排序。如果您有足够的权限，您也可以清除部分或全部日志记录。',
...
// Index page
$lang = array_merge($lang, array(
	'ADMIN_INTRO'				=> '感谢您选择 phpBB 作为论坛解决方案。这个面板将显示您论坛的总体统计信息。左边的链接允许您从各方面管理论坛，每个分页有如何使用管理工具的提示。',
	'ADMIN_LOG'					=> '记录的管理员操作',
	'ADMIN_LOG_INDEX_EXPLAIN'	=> '这里显示了管理员进行的最近五个操作。完整的操作日志可以通过下面的链接查看。',
	...
	'ERROR_MBSTRING_FUNC_OVERLOAD'					=> '重载函数配置错误',
	'ERROR_MBSTRING_FUNC_OVERLOAD_EXPLAIN'			=> '<var>mbstring.func_overload</var>必须设置为 0 或 4。您可以在 <samp>PHP信息</samp>页检查当前值。',
	'ERROR_MBSTRING_ENCODING_TRANSLATION'			=> '空白字符编码配置错误',
	'ERROR_MBSTRING_ENCODING_TRANSLATION_EXPLAIN'	=> '<var>mbstring.encoding_translation</var>必须设置为 0。您可以在 <samp>PHP信息</samp>页检查当前值。',
	'ERROR_MBSTRING_HTTP_INPUT'						=> 'HTTP 输入字符转换配置错误',
	'ERROR_MBSTRING_HTTP_INPUT_EXPLAIN'				=> '<var>mbstring.http_input</var>必须设置为<samp>pass</samp>。您可以在 <samp>PHP信息</samp>页检查当前值。',
	'ERROR_MBSTRING_HTTP_OUTPUT'					=> 'HTTP 输出字符转换配置错误',
	'ERROR_MBSTRING_HTTP_OUTPUT_EXPLAIN'			=> '<var>mbstring.http_output</var>必须设置为<samp>pass</samp>。您可以在 <samp>PHP信息</samp>页检查当前值。',
	...
	'PHP_VERSION'		=> 'PHP 版本',
	'PHP_VERSION_OLD'	=> '当前服务器上的 PHP 版本 (%1$s) 将不被未来的 phpBB 版本所支持。 最低要求的版本是   PHP %2$s. %3$s详细信息%4$s',
	...
	'VERSIONCHECK_INVALID_ENTRY'	=> '最新的版本信息包含一个不支持的记录。',
	'VERSIONCHECK_INVALID_URL'		=> '最新的版本信息包含一个无效的网址。',
	'VERSIONCHECK_INVALID_VERSION'	=> '最新的版本信息包含一个无效的版本号。',
	'VERSION_NOT_UP_TO_DATE_ACP'	=> '您安装的 phpBB 不是最新版的。<br /> 点击下面的链接可以查看更新版的发行公告，以获取升级的信息。',
	'VERSION_NOT_UP_TO_DATE_TITLE'	=> '您安装的 phpBB 不是最新版的。',
	'VERSION_UP_TO_DATE_ACP'	=> '您安装的 phpBB 已经是最新版的。暂时还没有升级版本可用。',
	...
	'WRITABLE_CONFIG'      => '您的论坛配置文件(config.php)处于可写状态。我们强烈建议您将此文件的权限设置为 640 或者至少为 644 (例如: <a href="http://en.wikipedia.org/wiki/Chmod" rel="external">chmod</a> 640 config.php)。',
));
...
$lang = array_merge($lang, array(
	...
	'INACTIVE_USERS_EXPLAIN'		=> '这是已经注册但是没有激活的用户列表。您可以手工激活，删除或提醒 (发送 email) 他们。',
	'INACTIVE_USERS_EXPLAIN_INDEX'	=> '这个列表包括最近 10 个已注册但未激活的用户，账户未激活可能是因为在用户注册设置里已启用账户激活，但这些用户还没被激活，也可能因为这些账户被禁用。点击下面的链接可得到一份完整的列表，您可以根据需要，来激活、删除或提醒（通过 email）这些用户。',
...
$lang = array_merge($lang, array(
	'EXPLAIN_SEND_STATISTICS'	=> '请发送您的服务器和论坛设置信息至 phpBB 官方以便于进行统计分析。此信息不会包含任何您或您的论坛的隐私数据。所有数据都是完全<strong>匿名</strong>的。我们会在今后的 phpBB 版本开发中参考收集到的信息。统计的结果会向公众公开，并提供给 PHP 语言项目的开发团队。',
	'EXPLAIN_SHOW_STATISTICS'	=> '使用下面的按钮您可以查看发送的内容。',
	'DONT_SEND_STATISTICS'		=> '返回管理员控制面板，如果您不希望发送统计信息给 phpBB。',
	'GO_ACP_MAIN'				=> '前往管理员控制面板首页',
	'HIDE_STATISTICS'			=> '隐藏细节',
	'SEND_STATISTICS'			=> '发送统计',
	'SEND_STATISTICS_LONG'		=> '发送统计信息',
	'SHOW_STATISTICS'			=> '显示详情',
	'THANKS_SEND_STATISTICS'	=> '感谢您提交信息。',
...
$lang = array_merge($lang, array(
	...
	'LOG_ACL_RESTORE_PERMISSIONS'		=> '<strong>恢复自己的权限，此前使用的权限是来自</strong><br />» %s ',
	...
	'LOG_ATTACH_FILEUPLOAD'		=> '<strong>幽灵文件上传至帖子</strong><br />» ID %1$d - %2$s',
	'LOG_ATTACH_ORPHAN_DEL'		=> '<strong>幽灵文件删除</strong><br />» %s',

	'LOG_BAN_EXCLUDE_USER'	=> '<strong>增加排除用户到封禁</strong> 原因： "<em>%1$s</em>"<br />» %2$s ',
	'LOG_BAN_EXCLUDE_IP'	=> '<strong>增加排除 IP 到封禁</strong> 原因： "<em>%1$s</em>"<br />» %2$s ',
	'LOG_BAN_EXCLUDE_EMAIL' => '<strong>增加排除 Email 到封禁 email</strong> 原因： "<em>%1$s</em>"<br />» %2$s ',
	'LOG_BAN_USER'			=> '<strong>封禁用户</strong> 原因： "<em>%1$s</em>"<br />» %2$s ',
	'LOG_BAN_IP'			=> '<strong>封禁 IP</strong> 原因： "<em>%1$s</em>"<br />» %2$s',
	'LOG_BAN_EMAIL'			=> '<strong>封禁 email</strong> 原因： "<em>%1$s</em>"<br />» %2$s',
	'LOG_UNBAN_USER'		=> '<strong>用户解禁</strong><br />» %s',
	'LOG_UNBAN_IP'			=> '<strong>IP 解禁</strong><br />» %s',
	'LOG_UNBAN_EMAIL'		=> '<strong>Email 解禁</strong><br />» %s',

	'LOG_BBCODE_ADD'		=> '<strong>添加新 BBCode</strong><br />» %s',
	'LOG_BBCODE_EDIT'		=> '<strong>编辑 BBCode</strong><br />» %s',
	'LOG_BBCODE_DELETE'		=> '<strong>删除 BBCode</strong><br />» %s',
	'LOG_BBCODE_CONFIGURATION_ERROR'	=> '<strong>当配置 BBCode 时的错误</strong>: %1$s<br />» %2$s',
	...
	'LOG_CONFIG_COOKIE'			=> '<strong>更改 cookie 设定</strong>',
	'LOG_CONFIG_EMAIL'			=> '<strong>更改 email 设定</strong>',
	...
	'LOG_CONFIG_FEED'			=> '<strong>更改 ATOM 设定</strong>',
	...
	'LOG_SPLIT_SOURCE'			=> '<strong>分割帖子</strong><br />» 自 %s',
	...
	'LOG_DOWNLOAD_EXCLUDE_IP'	=> '<strong>从允许下载列表中去除 IP/主机名</strong><br />» %s',
	'LOG_DOWNLOAD_IP'			=> '<strong>添加 IP/主机名到允许下载列表</strong><br />» %s',
	'LOG_DOWNLOAD_REMOVE_IP'	=> '<strong>从允许下载列表中删除 IP/主机名</strong><br />» %s',

	'LOG_ERROR_JABBER'		=> '<strong>Jabber 错误</strong><br />» %s',
	'LOG_ERROR_EMAIL'		=> '<strong>Email 错误</strong><br />» %s',
	'LOG_ERROR_CAPTCHA'		=> '<strong>CAPTCHA 错误</strong><br />» %s',	
	...
	'LOG_INSTALL_INSTALLED'	=> '<strong>安装 phpBB %s</strong>',

	'LOG_IP_BROWSER_FORWARDED_CHECK'	=> '<strong>对话 IP/browser/X_FORWARDED_FOR 检查失败</strong><br />»用户IP "<em>%1$s</em>" checked against 对话IP "<em>%2$s</em>"，用户浏览器字符串 "<em>%3$s</em>" checked against 对话浏览器字符串 "<em>%4$s</em>" and 用户 X_FORWARDED_FOR string "<em>%5$s</em>" checked against 对话 X_FORWARDED_FOR string "<em>%6$s</em>"。',

	'LOG_JAB_CHANGED'			=> '<strong>Jabber 帐号修改</strong>',
	'LOG_JAB_PASSCHG'			=> '<strong>Jabber 密码修改</strong>',
	'LOG_JAB_REGISTER'			=> '<strong>Jabber 帐号注册</strong>',
	'LOG_JAB_SETTINGS_CHANGED'	=> '<strong>Jabber 设置修改</strong>',
	...
	'LOG_MASS_EMAIL'		=> '<strong>群发 email</strong><br />» %s',
	...
	'LOG_WARNINGS_DELETED'		=> array(
		1 => '<strong>删除 %2$d 用户警告</strong><br />» %1$s', // Example: '<strong>Deleted 2 user warnings</strong><br />» username'
	),
	...
	'LOG_EXT_UPDATE'	=> '<strong>扩展已更新</strong><br />» %s',
));
